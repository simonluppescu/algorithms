...
const memos = {};
function largestSubArraySumHelper(arr, sum) {
    ...
    const result = Math.max(sum, largestSubArraySumHelper(arr.slice(1), sum + arr[0]), largestSubArraySumHelper(arr.slice(1), arr[0]));
    if (memos[key(arr, sum)] === undefined) {
