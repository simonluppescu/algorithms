...
class NotOptimalSolver {
  ...
  bitOutput(overriddenInput = null): number[] {
    if (overriddenInput) this.decimalOutput = overriddenInput;
  ...
  computeHelper(arr: number[], index: number, sum: number): number[] {
    if (index === -1) {
...
class OptimalSolver {
  ...
  computeInput(arr: number[]): number {
    let sum = 0;
  ...
  private truncate(num: number): number {
    if (num < 0) {
