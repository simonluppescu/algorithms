...
class Decoder {
  ...
  constructor() {
    this.seenValues = new Map<string, number>();
  }

  decodeVariations(str: string): number {
    if (this.seenValues[str]) return this.seenValues[str];
    if (str[0] === "0") return 0;
    if (str === "") return 1;
  ...
  isInRange(substr: string): boolean {
