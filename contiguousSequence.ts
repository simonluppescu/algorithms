...
function contiguous(arr: Array<number>): number {
  return contiguousHelper(arr, 0);
}

const memos: { [key: string]: number } = {};
function contiguousHelper(arr: Array<number>, sum: number): number {
  ...
  const result = Math.max(
    sum,
    contiguousHelper(arr.slice(1), sum + arr[0]),
    contiguousHelper(arr.slice(1), arr[0])
  );
