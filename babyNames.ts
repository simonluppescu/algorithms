...
class NameNode {
  ...
  private neighbors: Array<NameNode>;
...
class BabyNames {
  private nameMapper: Map<string, NameNode>;

  constructor() {
    this.nameMapper = new Map<string, NameNode>();
  }

  initMapper(names: Array<{ name: string; count: number }>, synonyms: Array<Array<string>>) {
  ...
  outputHelper(node: NameNode): number {
