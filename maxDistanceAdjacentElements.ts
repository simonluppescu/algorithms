...
function solution(arr: number[]): number {
  ...
  for (let i = 0; i < arr.length; i++) {
    if (indices[arr[i]] === undefined) indices[arr[i]] = [];
  ...
  for (let i = 1; i < values.length; i++) {
    ...
    const currDistance = Math.max(
      getDiff(first[0], second[second.length - 1]),
      getDiff(first[first.length - 1], second[0])
    );
...
function getDiff(first: number, second: number): number {
